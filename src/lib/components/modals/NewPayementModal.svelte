<script>
    ...
    onMount(async () => {
        const payersResponse = await axios.get('admin/payers', {
...
<Modal title="Vytvoření platby" bind:open size="xs" class="w-full">
    <form
        ...
        <Label class="space-y-2">
            ...
            <Select
                items={selectmenuPayers}
                bind:value={selected}
                placeholder="Vyber platiče"
                on:change={() => {
                    selectmenuPayers = selectmenuPayers.filter(
            ...
            {#each payers.filter( (item) => selectedPayers.includes(item.id) ) as payer}
                <ButtonGroup class="w-full pb-1">
                    <Button
                        on:click={() => {
                            console.log(
                                `${payer.id} ${payer.firstName} ${payer.lastName}`
                            );
                            selectedPayers = selectedPayers.filter(
                                (item) => item !== payer.id
                            );
                            selectmenuPayers = [
                                ...selectmenuPayers,
        ...
        <Button
            color="yellow"
            disabled={selectedPayers.length === 0 || !title || disabledSubmit}
            type="submit">
            <IconCash class="mr-1" />
            Vytvořit platbu
        </Button>
