<Modal title="Vytvoření pravidelné platby" bind:open={open} size="xs" class="w-full">
    <form class="flex flex-col space-y-6" on:submit|preventDefault={postPayment}>
        ...
        <Label class="space-y-2">
            <span>Cron</span>
            <ButtonGroup class="w-full">
                <Input type="text" placeholder="0 10 * * *"  bind:value={cronExpression}/>
            </ButtonGroup>
        </Label>
...
<script>
    ...
    let totalAmount;
    let amIPayer = true;
    let title, description, cronExpression = "";
    ...
    const postPayment = async (e) => {
        ...
        disabledSubmit = true;
        await axios.post(`admin/period-payments`, JSON.stringify({
            "title": title,
